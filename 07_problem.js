const cleanUpArray = (allAr) => {
    // let all = allAr.map(Number);
    // let evenAr = all.filter((n) => n % 2 === 0);
    // let oddAr = all.filter((n) => n % 2 === 1);

    // return [evenAr, oddAr];

    return allAr.map(Number).reduce(
        (acc, num) => {
             num % 2 ? acc[1].push(num) : acc[0].push(num);
             return acc;
        },
        [[], []]
    );
